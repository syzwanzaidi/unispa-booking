...
<div class="container mt-5">
    ...
    @else
        @foreach ($categorizedPackages as $category => $packageGroups)
            <h2 class="mt-4 mb-3 text-black">{{ $category }}</h2>
            <div class="row">
                @foreach ($packageGroups as $packageGroup)
                    <div class="col-md-4 mb-4">
                        <div class="card h-100">
                            <div class="card-body">
                                <h5 class="card-title text-center text-black">{{ $packageGroup->package_name }}</h5>
                                    ...
                                    @foreach ($packageGroup->options as $option)
                                        <li class="list-group-item d-flex justify-content-between align-items-center text-black">
                                            <div>
                                                ...
                                                <br><small class="text-dark fw-bold">Capacity: {{ $option->capacity }}</small>
                                            </div>
                                            @if (Auth::guard('web')->check())
                                                <a href="{{ route('bookings.create', ['package_id' => $option->package_id]) }}" class="btn btn-sm btn-primary text-black">Book Now</a>
                                            @elseif (Auth::guard('admin')->check())
                                            @else
                                                <a href="{{ route('login') }}" class="btn btn-sm btn-outline-primary text-black">Login to Book</a>
                                            @endif
                                        </li>
                                    @endforeach
                                </ul>
